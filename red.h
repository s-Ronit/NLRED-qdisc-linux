				 ...
				 u8 Scell_log, u8 *stab, u32 max_P)
{
	...
	// p->max_P = max_P;    // RED CODE WITHOUT CHANGE 
	p->max_P = max_P + (max_P >> 1); /* max_p = 1.5 * max_p NL RED
	
	printk(KERN_ALERT "THIS IS RED.H second time");
	pr_info(KERN_ALERT "THIS IS RED.H second time");
...
{
	printk(KERN_ALERT "13");
	return reciprocal_divide(get_random_u32(), p->max_P_reciprocal);
}

static inline int red_mark_probability(const struct red_parms *p,const struct red_vars *v,unsigned long qavg)
{
	 ...
	 */
	// return !(((qavg - p->qth_min) >> p->Wlog) * v->qcount < v->qR);  // THIS IS RED
	return !((mult_frac((qavg - p->qth_min) >> p->Wlog, (qavg - p->qth_min) >> p->Wlog, p->max_P)) * v->qcount < v->qR);
	// NL-RED
